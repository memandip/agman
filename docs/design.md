...
`create <name>` defaults to copying `~/.claude` (explicit: `--from-current`; alternatives: `--empty`, `--from <profile>`). Copy uses an **exclude list** (not an include list) so future config files are picked up by default. Excluded as ephemeral/machine-local: `antigravity antigravity-browser-profile backups cache downloads file-history history ide locks log logs paste-cache projects session-env sessions shell-snapshots statsig tasks telemetry tmp .tmp todos history.jsonl mcp-needs-auth-cache.json .last-cleanup .DS_Store`. Notably `projects/` (per-project auto-memory + transcripts) is excluded: fresh memory per profile, and personal history never leaks into a work profile. The `antigravity*` names cover state that other products drop into the managed trees — Antigravity keeps a multi-GB browser profile inside `~/.gemini` — which would otherwise bloat every seed and cloud push. Shared-state merge leftovers (`*.agman-conflict`, `*.agman-orphaned` — per-machine copies parked by state adoption) are also excluded from seeding. Dependency artifacts (`.venv`/`venv`, `node_modules`, `__pycache__`, `.git`) **do** seed locally — a copied venv still runs because the active profile is always symlinked at the tool's real config path — but are excluded from cloud sync, where they are reinstallable weight.
