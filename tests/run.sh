#!/usr/bin/env bash
#
# ccprofile test suite. No dependencies beyond bash + coreutils.
# Runs fully sandboxed: fake $HOME, fake claude binary, temp CCPROFILE_HOME.

set -u

ROOT="$(cd "$(dirname "$0")/.." && pwd -P)"
CCP="$ROOT/bin/ccprofile"
...
export CCPROFILE_HOME="$HOME/.claude-profiles"
unset CLAUDE_CONFIG_DIR CCPROFILE_SHELL_INTEGRATION CCPROFILE_CLAUDE_HOME 2>/dev/null || true
...
assert_eq   "seeded CLAUDE.md content" "GLOBAL RULES" "$(cat "$CCPROFILE_HOME/work/CLAUDE.md")"
assert_exists  "seeded settings.json" "$CCPROFILE_HOME/work/settings.json"
assert_exists  "seeded skills dir" "$CCPROFILE_HOME/work/skills/foo.md"
assert_exists  "seeded user .claude.json (MCP/global state)" "$CCPROFILE_HOME/work/.claude.json"
assert_missing "sessions excluded from seed" "$CCPROFILE_HOME/work/sessions"
assert_fail "duplicate create rejected" "$CCP" create work

assert_ok   "create --empty" "$CCP" create personal --empty
assert_missing "empty profile has no CLAUDE.md" "$CCPROFILE_HOME/personal/CLAUDE.md"

assert_ok   "create --from clones a profile" "$CCP" create staging --from work
assert_eq   "clone carries CLAUDE.md" "GLOBAL RULES" "$(cat "$CCPROFILE_HOME/staging/CLAUDE.md")"

assert_ok   "create --link-plugins symlinks plugins" "$CCP" create linked --link-plugins
if [ -L "$CCPROFILE_HOME/linked/plugins" ]; then ok "plugins is a symlink"; else bad "plugins is a symlink"; fi
...
assert_eq "dir --active resolves default" "$CCPROFILE_HOME/work" "$("$CCP" dir --active)"
assert_eq "dir <name> prints path" "$CCPROFILE_HOME/personal" "$("$CCP" dir personal)"
...
assert_eq "env line is eval-able" "$CCPROFILE_HOME/work" \
...
out="$(bash -c 'eval "$(ccprofile init bash)"; ccprofile use personal >/dev/null; claude hi')"
assert_contains "hook: use exports into current shell" "$out" "CONFIG_DIR=$CCPROFILE_HOME/personal"

out="$(bash -c 'eval "$(ccprofile init bash)"; claude hi')"
assert_contains "hook: claude wrapper honors persistent default" "$out" "CONFIG_DIR=$CCPROFILE_HOME/personal"

out="$(bash -c 'eval "$(ccprofile init bash)"; ccprofile off >/dev/null; claude hi')"
assert_contains "hook: off returns claude to stock" "$out" "CONFIG_DIR=unset"

out="$(bash -c 'eval "$(ccprofile init bash)"; CLAUDE_CONFIG_DIR=/explicit claude hi')"
...
assert_contains "version prints" "$("$CCP" version)" "ccprofile"
