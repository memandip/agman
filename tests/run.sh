#!/usr/bin/env bash
#
# agman test suite. No dependencies beyond bash + coreutils.
...
unset CLAUDE_CONFIG_DIR AGMAN_SHELL_INTEGRATION AGMAN_CLAUDE_HOME 2>/dev/null || true
...
assert_exists  "seeded skills dir" "$AGMAN_HOME/work/skills/foo.md"
assert_exists  "seeded user .claude.json (MCP/global state)" "$AGMAN_HOME/work/.claude.json"
...
assert_missing "empty profile has no CLAUDE.md" "$AGMAN_HOME/personal/CLAUDE.md"
...
assert_eq "dir --active resolves default" "$AGMAN_HOME/work" "$("$CCP" dir --active)"
assert_eq "dir <name> prints path" "$AGMAN_HOME/personal" "$("$CCP" dir personal)"
...
out="$(CLAUDE_CONFIG_DIR="$AGMAN_HOME/personal" "$CCP" current)"
...
assert_eq "default follows rename" "$AGMAN_HOME/office" "$("$CCP" dir --active)"
...
assert_contains "version prints" "$("$CCP" version)" "agman"
